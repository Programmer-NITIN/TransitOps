...
exports.list = async (req, res, next) => { try { if (req.user.role === 'Driver') { req.query.driver_id = req.user.id; } const d = await tripService.list(req.query); res.json({ success: true, data: d, count: d.length }); } catch (e) { next(e); } };
